...
class OpenAPIGenerator:
    ...
    def generate_with_progress(self, spec_file: str, output_dir: str, progress: Progress, task_id: TaskID):
        ...
        cmd = [
            ...
            '-g', 'spring',
            '-o', temp_dir,
            '--api-package', f"{self.config.java_package_base}.{self.config.api_package}",
            '--model-package', f"{self.config.java_package_base}.{self.config.model_package}",
        ]
        subprocess.run(cmd, check=True, capture_output=True)

        # Process generated files
        progress.update(task_id, description="[yellow]Processing generated files...")
        model_dir = os.path.join(temp_dir, 'src/main/java',
                               self.config.java_package_base.replace('.', '/'),
